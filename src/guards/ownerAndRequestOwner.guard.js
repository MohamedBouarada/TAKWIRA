...
async function ownerAndRequestOwnerGuard(req,res,next){
    try{
        const {authEmail,authId,authRole} = req.infos;
        if(authRole!==UserType.OwnerRequest && authRole !==UserType.Owner) {
        ...
        if(userExists.data.role !==UserType.OwnerRequest && userExists.data.role!==UserType.Owner) {
...
module.exports = ownerAndRequestOwnerGuard